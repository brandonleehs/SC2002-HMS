...
/**
 * The PharmacistRepository class manages the collection of pharmacist records.
 * It provides methods for accessing, modifying, and serializing pharmacist
 * data. The data is stored in a map with the pharmacist's ID as the key and the
...
public class PharmacistRepository implements IUserRepository<Pharmacist> {
	...
	/**
	 * Constructor that initializes the PharmacistRepository by deserializing
	 * pharmacist data from a CSV file.
	 * 
	 ...
	 * @return the pharmacist associated with the specified ID, or {@code null} if
	 *         no pharmacist exists with that ID
	 */
	@Override
	...
	/**
	 * Serializes the pharmacist data and appends it to a CSV file. The data
	 * includes pharmacist ID, name, role, gender, age, and password hash.
	 * 
	 */
	@Override
