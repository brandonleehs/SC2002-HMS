...
/**
 * The PatientRepository class manages the collection of patient records, providing methods for
 * accessing, modifying, and serializing patient data.
 * It uses a map to store patients, where the key is the patient ID and the value is the patient object.
 */
public class PatientRepository implements IUserRepository<Patient> {
	...
	/**
     * Constructor that initializes the PatientRepository by deserializing patient data from a CSV file.
     * 
     ...
     * @return the patient associated with the specified ID, or {@code null} if no patient exists with that ID
     */
	@Override
	...
	/**
     * Serializes the patient data and writes it to a CSV file. 
     * The data includes patient ID, name, date of birth, gender, blood type, contact information,
     * phone number, and password hash.
     * 
     * @throws FileNotFoundException if the file path is invalid or the file cannot be written
     */
	@Override
